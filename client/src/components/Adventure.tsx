...
const Adventure = () => {
  ...
  const loadMoreItems = () => {
    // Increase the display count by a certain amount when "View More" is clicked
    setDisplayCount(displayCount + 3); // You can adjust this number according to your preference
  };
  return (
    <div className="flex md:flex flex-col justify-center overflow-hidden  w-screen items-center ">
      <div className=" flex justify-center items-center pt-8 font-normal text-[30px] text-center">
        <h1>From platformers to adventure to RPGs. Game on.</h1>
      </div>
      <div className="flex justify-center items-center  pt-3 gap-3">
        <h1 className="font-bold ">Sorted by:</h1>
        <h3 className="font-[300] cursor-pointer">Realised Date</h3>
        <h3 className="font-[300] cursor-pointer">Name</h3>
      </div>
      <div
        id="slider"
        className=" flex  justify-center flex-wrap   pt-10 sm:w-[90%]  md:w-[90%] xl:w-[90%] lg:[70%] h-auto gap-10"
      >
        {items.slice(0, displayCount).map((item) => (
          <div key={item.id} className="">
            <div className="flex  border-[6px] border-slate-200 flex-wrap gap-6 w-40 h-20 md:w-50 md:h-20 items-center  bg-green-500 lg:w-[300px]  hover:border-hidden lg:h-[80px] xl:w-[300px] xl:h-[150px] transition-transform duration-300 transform hover:scale-11 group">
              <Image
                src={item.img}
                alt="Play on Stream"
                fill
      ...
      {displayCount < items.length && (
        <div className="flex justify-center items-center text-center pt-6">
          <button
            className="rounded bg-white p-2 items-center w-auto border-slate-500 border-[2px] justify-center flex md:w-[200px] text-center tsm:w-auto h-[30px]"
